...
type HTTPOriginProxy interface {
	// RoundTripper is how netscale proxies eyeball requests to the actual origin services
	http.RoundTripper
}
...
// HTTPLocalProxy can be implemented by netscale services that want to handle incoming http requests.
type HTTPLocalProxy interface {
	// Handler is how netscale proxies eyeball requests to the local netscale services
	http.Handler
}
