...
import (
	...
	"github.com/khulnasoft/netscale/ingress/middleware"
	"github.com/khulnasoft/netscale/ipaccess"
)

var (
	ErrNoIngressRules             = errors.New("The config file doesn't contain any ingress rules")
	ErrNoIngressRulesCLI          = errors.New("No ingress rules were defined in provided config (if any) nor from the cli, netscale will return 503 for all incoming HTTP requests")
...
//
// Negative index rule signifies local netscale rules (not-user defined).
...
// StartOrigins will start any origin services managed by netscale, e.g. proxy servers or Hello World.
