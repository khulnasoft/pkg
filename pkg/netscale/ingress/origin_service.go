...
type OriginService interface {
	String() string
	// Start the origin service if it's managed by netscale, e.g. proxy servers or Hello World.
	// If it's not managed by netscale, this is a no-op because the user is responsible for
	// starting the origin service.
	// Implementor of services managed by netscale should terminate the service if shutdownC is closed
...
// Users only use this for testing and experimenting with netscale.
...
// default status code (503) that is returned for requests to netscale that don't have any ingress rules setup
