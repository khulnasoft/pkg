...
// HTTP2Connection represents a net.Conn that uses HTTP2 frames to proxy traffic from the edge to netscale on the
// origin.
...
func (c *HTTP2Connection) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case TypeTCP:
		...
		if err != nil {
			requestErr = fmt.Errorf(`netscale received a warp-routing request with an empty host value: %w`, err)
			break
		}
...
func (rp *http2RespWriter) WriteRespHeaders(status int, header http.Header) error {
	...
	for name, values := range header {
		...
		if h2name == tracing.IntNetscaleTracingHeader {
			// Add cf-int-netscale-tracing header outside of serialized userHeaders
			dest[tracing.CanonicalNetscaleTracingHeader] = values
			continue
		}
