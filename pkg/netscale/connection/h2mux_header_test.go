...
func TestH2RequestHeadersToH1Request_WeirdURLs(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			// https://github.com/khulnasoft/netscale/issues/81
			path: "//test/",
			want: "//test/",
		},
		{
		...
		{
			// https://github.com/khulnasoft/netscale/issues/124
...
func TestIsH2muxControlResponseHeader(t *testing.T) {
	controlResponseHeaders := []string{
		// Anything that begins with cf-int- or cf-netscale-
		"cf-int-sample-header",
		"cf-netscale-sample-header",
...
func TestIsNotH2muxControlResponseHeader(t *testing.T) {
	notControlResponseHeaders := []string{
		...
		"upgrade",
		"connection",
		"cf-whatever", // On the response path, we only want to filter cf-int- and cf-netscale-
	}
...
func TestH1ResponseToH2ResponseHeaders(t *testing.T) {
	mockHeaders := http.Header{
		...
		"cf-netscale-header": {"cf-netscale-value"},
		"Content-Length":        {"123"},
	}
