...
$Go.import("github.com/khulnasoft/netscale/tunnelrpc");
...
struct RegistrationOptions {
    ...
    # Set of features this netscale knows it supports
...
interface SessionManager {
    # Let the edge decide closeAfterIdle to make sure netscale doesn't close session before the edge closes its side
...
struct UpdateConfigurationResponse {
    ...
    # that netscale is using an older configuration because the latest cannot be applied
    latestAppliedVersion @0 :Int32;
    # Any error encountered when trying to apply the last configuration
    err @1 :Text;
}

# ConfigurationManager defines RPC to manage netscale configuration remotely
interface ConfigurationManager {
    updateConfiguration @0 (version :Int32, config :Data) -> (result: UpdateConfigurationResponse);
}

interface NetscaleServer extends(SessionManager, ConfigurationManager) {}