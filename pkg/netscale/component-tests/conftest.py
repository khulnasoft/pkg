        ...
        def _component_tests_config(additional_config={}, cfd_mode=CfdModes.NAMED, run_proxy_dns=True, provide_ingress=True):
            ...
            if cfd_mode is CfdModes.NAMED:
                return NamedTunnelConfig(additional_config=additional_config,
                                         netscale_binary=config['netscale_binary'],
                                         tunnel=config['tunnel'],
                                         credentials_file=config['credentials_file'],
                                         ingress=ingress,
                                         hostname=hostname)
            elif cfd_mode is CfdModes.PROXY_DNS:
                return ProxyDnsConfig(netscale_binary=config['netscale_binary'])
            elif cfd_mode is CfdModes.QUICK:
                return QuickTunnelConfig(additional_config=additional_config, netscale_binary=config['netscale_binary'])
            else:
                raise Exception(f"Unknown netscale mode {cfd_mode}")

        return _component_tests_config


# This fixture is automatically called before each tests to make sure the previous netscale has been shutdown
@pytest.fixture(autouse=True)
def wait_previous_netscale():
    sleep(BACKOFF_SECS)
