...
class TestTermination:
    ...
    def test_graceful_shutdown(self, tmp_path, component_tests_config, signal, protocol):
        ...
        with start_netscale(
                tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1"],  new_process=True, capture_output=False) as netscale:
            ...
            self.terminate_by_signal(netscale, signal)
            self.wait_eyeball_thread(
                in_flight_req, self.grace_period + self.timeout)

    # test netscale terminates before grace period expires when all eyeball
    ...
    def terminate_by_signal(self, netscale, sig):
        netscale.send_signal(sig)
