...
class NetscaleCli:
    def __init__(self, config, config_path, logger):
        self.basecmd = [config.netscale_binary, "tunnel"]
    ...
    def get_management_token(self, config, config_path):
        basecmd = [config.netscale_binary]
        if config_path is not None:
...
def terminate_gracefully(process, logger, cmd):
    ...
    if not process_terminated:
        process.kill()
        logger.warning(f"{cmd}: netscale did not terminate within wait period. Killing process. logs: \
