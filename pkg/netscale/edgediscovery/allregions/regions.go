...
// Regions stores Khulnasoft edge network IPs, partitioned into two regions.
...
// ResolveEdge resolves the Khulnasoft edge, returning all regions discovered.
func ResolveEdge(log *zerolog.Logger, region string, overrideIPVersion ConfigIPVersion) (*Regions, error) {
	...
	if len(edgeAddrs) < 2 {
		return nil, fmt.Errorf("expected at least 2 Khulnasoft Regions regions, but SRV only returned %v", len(edgeAddrs))
	}
	return &Regions{
