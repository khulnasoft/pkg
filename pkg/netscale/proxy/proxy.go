...
func (p *Proxy) ProxyTCP(
	...
	if p.warpRouting == nil {
		err := errors.New(`netscale received a request from WARP client, but your configuration has disabled ingress from WARP clients. To enable this, set "warp-routing:\n\t enabled: true" in your config.yaml`)
...
func (p *Proxy) proxyHTTPRequest(
	...
	if err != nil {
		...
		return errors.Wrap(err, "Unable to reach the origin service. The service may be down or it may not be responding to traffic from netscale")
	}
...
func (p *Proxy) logRequest(r *http.Request, fields logFields) {
	...
	if fields.cfRay == "" && !fields.lbProbe {
		log.Debug().Msgf("All requests should have a CF-RAY header. Please open a support ticket with Khulnasoft. %s %s %s ", r.Method, r.URL, r.Proto)
	}
	event.
