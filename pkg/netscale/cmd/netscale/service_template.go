...
func serviceAlreadyExistsWarn(service string) string {
	return fmt.Sprintf("netscale service is already installed at %s; if you are running a netscale tunnel, you "+
		"can point it to multiple origins, avoiding the need to run more than one netscale service in the "+
		"same machine; otherwise if you are really sure, you can do `netscale service uninstall` to clean "+
