...
// Example: assigning LogEventType.Netscale to a zerolog event will allow the client to filter for only
// the Netscale-related events.
...
// the the first two are limited to failure conditions that lead to netscale shutting down.
type LogLevel int8
