...
func (ch *Checker) Check(ctx context.Context) (*doghouse.CheckResponse, error) {
	...
	if err != nil {
		// If this error is StatusForbidden (403) here, it means codebase is
		...
		// is invalid, codebase should return an error earlier (e.g. when reading
...
func (ch *Checker) checkTitle() string {
	if name := ch.checkName(); name != "codebase" {
		return fmt.Sprintf("codebase [%s] report", name)
	}
...
func (ch *Checker) summary(checks []*filter.FilteredDiagnostic) string {
	var lines []string
	var usedBytes int
	lines = append(lines, "reported by [codebase](https://github.com/khulnasoft/codebase) :dog:")
...
func annotationToDiagnostic(a *doghouse.Annotation) *rdf.Diagnostic {
	...
	// Old codebase CLI doesn't have the Diagnostic field.
