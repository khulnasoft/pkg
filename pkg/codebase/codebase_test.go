...
`
	...
	app := NewCodebase("tool name", p, c, d, filter.ModeAdded, false)
...
func TestCodebase_Run_clean_path(t *testing.T) {
	difftext := `diff --git a/golint.old.go b/golint.new.go
index 34cacb9..a727dd3 100644
--- a/golint.old.go
+++ b/golint.new.go
...
func TestCodebase_Run_returns_nil_if_fail_on_error_not_passed_and_some_errors_found(t *testing.T) {
	difftext := `diff --git a/golint.old.go b/golint.new.go
index 34cacb9..a727dd3 100644
--- a/golint.old.go
+++ b/golint.new.go
...
func TestCodebase_Run_returns_error_if_fail_on_error_passed_and_some_errors_found(t *testing.T) {
	difftext := `diff --git a/golint.old.go b/golint.new.go
index 34cacb9..a727dd3 100644
--- a/golint.old.go
+++ b/golint.new.go
...
`
	...
	p := parser.NewErrorformatParser(efm)
	d := NewDiffString(difftext, 1)
	app := NewCodebase("tool name", p, c, d, filter.ModeAdded, true)
	err := app.Run(context.Background(), strings.NewReader(lintresult))
