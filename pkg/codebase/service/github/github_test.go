...
const notokenSkipTestMes = "skipping test (requires actual Personal access tokens. export CODEBASE_TEST_GITHUB_API_TOKEN=<GitHub Personal Access Token>)"

func setupGitHubClient() *github.Client {
	token := os.Getenv("CODEBASE_TEST_GITHUB_API_TOKEN")
	if token == "" {
		return nil
	}
...
+
+func NewCodebase(p Parser, c CommentService, d DiffService) *Codebase {
+	return &Codebase{p: p, c: c, d: d}
+}
+
