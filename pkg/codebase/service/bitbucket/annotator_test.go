...
func (s *AnnotatorTestSuite) TestManyComments() {
	...
	comments := make([]*codebase.Comment, 333)
...
func (s *AnnotatorTestSuite) buildReportReq(runner string, result string) *ReportRequest {
	...
	case reportResultPassed:
		report.Details = "Great news! Codebase couldn't spot any issues!"
	case reportResultPending:
		report.Details = "Please wait for Codebase to finish checking your code for issues."
	default:
		report.Details = "Woof-Woof! This report generated for you by codebase."
	}

	return report
}

func (s *AnnotatorTestSuite) buildAnnotationsRequest(runner string, comments []*codebase.Comment) *AnnotationsRequest {
...
func (s *AnnotatorTestSuite) buildComment(toolName string, line int32) *codebase.Comment {
	return &codebase.Comment{
		ToolName: toolName,
...
func (s *AnnotatorTestSuite) splitComments(comments []*codebase.Comment) map[string][]*codebase.Comment {
	commentsMap := make(map[string][]*codebase.Comment)
