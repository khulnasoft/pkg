...
var _ codebase.CommentService = &ReportAnnotator{}

const (
	// avatar from https://github.com/apps/codebase
...
type ReportAnnotator struct {
	...
	comments map[string][]*codebase.Comment
...
func NewReportAnnotator(cli APIClient, owner, repo, sha string, runners []string) *ReportAnnotator {
	r := &ReportAnnotator{
		...
		owner:      owner,
		repo:       repo,
		comments:   make(map[string][]*codebase.Comment, len(runners)),
		duplicates: map[string]struct{}{},
	}
	...
	for _, runner := range runners {
		...
		r.comments[runner] = []*codebase.Comment{}
...
// Bitbucket in batch.
func (r *ReportAnnotator) Post(_ context.Context, c *codebase.Comment) error {
...
func (r *ReportAnnotator) createOrUpdateReport(ctx context.Context, id, title, reportStatus string) error {
	...
	case reportResultPassed:
		req.Details = "Great news! Codebase couldn't spot any issues!"
	case reportResultPending:
		req.Details = "Please wait for Codebase to finish checking your code for issues."
	default:
		req.Details = "Woof-Woof! This report generated for you by codebase."
	}
