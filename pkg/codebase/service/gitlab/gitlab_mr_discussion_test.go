...
func TestGitLabMergeRequestDiscussionCommenter_Post_Flush_review_api(t *testing.T) {
	...
	alreadyCommented1 := &codebase.Comment{
	...
	alreadyCommented2 := &codebase.Comment{
		Result: &filter.FilteredDiagnostic{
			Diagnostic: &rdf.Diagnostic{
				Location: &rdf.Location{
	...
	commentOutsideDiff := &codebase.Comment{
		Result: &filter.FilteredDiagnostic{
			Diagnostic: &rdf.Diagnostic{
				Location: &rdf.Location{
					Path: "path.go",
	...
	commentWithoutLnum := &codebase.Comment{
		Result: &filter.FilteredDiagnostic{
			Diagnostic: &rdf.Diagnostic{
				Location: &rdf.Location{
					Path: "path.go",
	...
	newCommentWithSuggestion := &codebase.Comment{
		Result: &filter.FilteredDiagnostic{
			Diagnostic: &rdf.Diagnostic{
				Location: &rdf.Location{
					Path: "file3.go",
...
func buildTestComment(message string, suggestions []*rdf.Suggestion) *codebase.Comment {
