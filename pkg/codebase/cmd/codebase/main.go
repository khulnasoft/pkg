...
import (
	...
	giteaservice "github.com/khulnasoft/codebase/service/gitea"
...
const (
	...
	efmsDoc       = `list of supported machine-readable format and errorformat (https://github.com/khulnasoft/codebase/errorformat)`
...
`
	reporterDoc = `reporter of codebase results. (local, github-check, github-pr-check, github-pr-reviewe, gitlab-mr-discussion, gitlab-mr-commit, gitea-pr-review)
	"local" (default)
	...
	"gitea-pr-review"
		...
		1. Set CODEBASE_GITEA_API_TOKEN environment variable.
...
func giteaService(ctx context.Context, opt *option) (gs *giteaservice.PullRequest, isPR bool, err error) {
	token, err := nonEmptyEnv("CODEBASE_GITEA_API_TOKEN")
	if err != nil {
		return nil, isPR, err
	}
...
func getGiteaPullRequestIDByBranchOrCommit(client *gitea.Client, info *cienv.BuildInfo) (int64, error) {
	...
	return 0, fmt.Errorf("codebase: PullRequest not found, query: %s", strings.Join(query, " "))
}
