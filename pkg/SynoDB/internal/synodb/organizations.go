...
func (c *OrganizationsClient) InviteMember(email, role string) error {
	if c.client.Org == "" {
		return fmt.Errorf("the currently active organization %s does not allow members. You can use %s to change to an active organization", internal.Emph("personal"), internal.Emph("synodb org switch"))
	}
	prefix := "/v1/organizations/" + c.client.Org
...
func notMemberErr(org string) error {
	...
	msg += fmt.Sprintf("%s is now configured to use your personal organization.", internal.Emph("synodb"))
