...
func getURL(db *synodb.Database, client *synodb.Client) (string, error) {
...
var shellCmd = &cobra.Command{
	...
	Example:           "  synodb db shell http://127.0.0.1:8080\n  synodb db shell name-of-my-amazing-db\n  synodb db shell name-of-my-amazing-db --location yyz\n  synodb db shell name-of-my-amazing-db --instance a-specific-instance\n  synodb db shell name-of-my-amazing-db \"select * from users;\"",
...
func databaseFromName(str string, client *synodb.Client) (*synodb.Database, error) {
	name := str
...
func tokenFromDb(db *synodb.Database, client *synodb.Client) (string, error) {
	if db == nil {
		return "", nil
	}
...
func getConnectionInfo(nameOrUrl string, db *synodb.Database) string {
	...
	msg += "\n\n"
	msg += "Welcome to Synodb SQL shell!\n\n"
