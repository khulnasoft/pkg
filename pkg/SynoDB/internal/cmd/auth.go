...
var authCmd = &cobra.Command{
	Use:               "auth",
	Short:             "Authenticate with Synodb",
...
var signupCmd = &cobra.Command{
	Use:               "signup",
	Short:             "Create a new Synodb account.",
	Args:              cobra.NoArgs,
...
var tokenCmd = &cobra.Command{
	Use:   "token",
	Short: "Shows the token used to authenticate you to Synodb platform API.",
	Long: "" +
		"Shows the token used to authenticate you to Synodb platform API.\n" +
		"To authenticate to your databases, use " + internal.Emph("synodb db tokens create"),
	Args:              cobra.NoArgs,
	ValidArgsFunction: noFilesArg,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !isJwtTokenValid(token) {
			return fmt.Errorf("no user logged in. Run %s to log in and get a token", internal.Emph("synodb auth login"))
		}

		fmt.Fprintln(os.Stderr, internal.Warn("Warning: this token is used to authenticate you to Synodb platform API, not your databases."))
		fmt.Fprintf(os.Stderr, "%s %s %s\n", internal.Warn("Use"), internal.Emph("synodb db tokens create"), internal.Warn("to create a database token."))
...
func beginAuth(port int, headless bool, path string) (string, error) {
	authUrl, err := url.Parse(fmt.Sprintf("%s%s", getSynodbUrl(), path))
	if err != nil {
