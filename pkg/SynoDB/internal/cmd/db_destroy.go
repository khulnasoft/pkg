...
func handleDestroySingleDB(args []string, client *synodb.Client) error {
	name := args[0]
	...
	if locationFlag != "" {
		if db.Group != "" {
			return fmt.Errorf("group databases do not support location destruction.\nUse %s instead", internal.Emph("synodb group locations rm "+name+" "+locationFlag))
		}
...
func handleDestroyMultipleDBs(args []string, client *synodb.Client) error {
