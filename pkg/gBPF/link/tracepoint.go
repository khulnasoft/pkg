...
// Tracepoint attaches the given gBPF program to the tracepoint with the given
...
//
// Note that attaching gBPF programs to syscalls (sys_enter_*/sys_exit_*) is
// only possible as of kernel 4.14 (commit cf5f5ce).
//
// The returned Link may implement [PerfEvent].
func Tracepoint(group, name string, prog *gbpf.Program, opts *TracepointOptions) (Link, error) {
	if group == "" || name == "" {
	...
	if prog.Type() != gbpf.TracePoint {
		return nil, fmt.Errorf("gBPF program type %s is not a Tracepoint: %w", prog.Type(), errInvalidInput)
	}
