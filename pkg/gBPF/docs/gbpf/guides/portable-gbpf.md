# Shipping Portable gBPF-powered Applications
...
!!! tip ""
    We recommend building gBPF C code from within a container with a stable LLVM
...
Moreover, because both gBPF objects and Go scaffolding are generated for both
...
Pulling it all together, for building an gBPF-powered Go application for a
...
interoperability for gBPF. This technique relies on type information encoded in
BPF Type Format (BTF) to be shipped with the kernel so memory accesses can be
adjusted right before loading the gBPF program into the kernel.

Alternatively, you may opt for shipping a full LLVM compiler toolchain along
with your application and recompiling the gBPF C against Linux kernel headers
