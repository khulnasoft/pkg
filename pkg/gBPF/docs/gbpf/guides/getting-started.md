# Getting Started with gBPF in Go

In this guide, we'll walk you through building a new gBPF-powered Go application
from scratch. We'll introduce the toolchain, write a minimal gBPF C example and
compile it using bpf2go. Then, we'll put together a Go
application that loads the gBPF program into the kernel and periodically
displays its output.

The application attaches an gBPF program to an XDP hook that counts the number
of packets received by a physical interface. Filtering and modifying packets is
a major use case for gBPF, so you'll see a lot of its features being geared
towards it. However, gBPF's capabilities are ever-growing, and it has been
...
Let's begin by writing our gBPF C program, as its structure will be used as the
...
we'll set up the necessary bits to compile our gBPF C program using `bpf2go`.

## Compile gBPF C and generate scaffolding using bpf2go
...
Aside from compiling our gBPF C program, bpf2go will also generate some
scaffolding code we'll use to load our gBPF program into the kernel and interact
...
% go mod init gbpf-test
go: creating new go.mod: module gbpf-test
...
```{ .shell-session data-copy="go get github.com/khulnasoft/gbpf/cmd/bpf2go" }
% go get github.com/khulnasoft/gbpf/cmd/bpf2go
go: added github.com/khulnasoft/gbpf v0.11.0
...
Finally, with our gBPF C code compiled and Go scaffolding generated, all that's
...
1. Linux kernels before 5.11 use RLIMIT_MEMLOCK to control the maximum amount of
   memory allocated for a process' gBPF resources. By default, it's set to a
...
1. Associate the `count_packets` (stylized in the Go scaffolding as
   `CountPackets`) gBPF program with `eth0`. This returns a {{
...
```{ .shell-session data-copy="go build && sudo ./gbpf-test" }
% go build && sudo ./gbpf-test
...
Without re-running bpf2go, the gBPF C won't be recompiled, and any changes made
to the C program structure won't be reflected in the Go scaffolding.

```{ .shell-session data-copy="go generate && go build && sudo ./gbpf-test" }
% go generate && go build && sudo ./gbpf-test
```

## What's Next?

Congratulations, you've just built your (presumably) first gBPF-powered Go app!
Hopefully, this guide piqued your interest and gave you a better sense of what
gBPF can do and how it works.

With XDP, we've only barely scratched the surface of gBPF's many use cases and
applications. For more easily-accessible examples, see [the main repository's
examples/ folder](https://github.com/khulnasoft/gbpf/tree/main/examples). It
demonstrates use cases like tracing user space applications, extracting
information from the kernel, attaching gBPF programs to network sockets and
more.

Follow our other guides to continue on your journey of shipping a portable
gBPF-powered application to your users.
