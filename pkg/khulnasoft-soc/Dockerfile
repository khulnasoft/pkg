FROM ghcr.io/khulnasoft/golang:1.20.1-alpine as builder
ARG VERSION=0.0.0
RUN apk update && apk add libpcap-dev bash git musl-dev gcc npm python3 py3-pip py3-virtualenv
COPY . /build
WORKDIR /build
RUN mkdir gitdocs && cd gitdocs && \
	git clone --no-single-branch --depth 50 https://github.com/khulnasoft/khulnasoft-documentation.git . && \
...
FROM ghcr.io/khulnasoft/python:3-slim
...
RUN bash -c "[[ $VERSION == '0.0.0' ]]" || \
    wget https://github.com/khulnasoft/khulnasoft-documentation/raw/$(echo $VERSION | cut -d'.' -f 1,2)/images/cheat-sheet/KhulnaSoft-Cheat-Sheet.pdf -O html/docs/cheatsheet.pdf
