...
import (
	...
	"go.khulnasoft.com/pkg/commands/artifact"
	"go.khulnasoft.com/pkg/commands/auth"
	"go.khulnasoft.com/pkg/commands/clean"
	"go.khulnasoft.com/pkg/commands/convert"
	"go.khulnasoft.com/pkg/commands/server"
	"go.khulnasoft.com/pkg/fanal/analyzer"
	"go.khulnasoft.com/pkg/flag"
	k8scommands "go.khulnasoft.com/pkg/k8s/commands"
	"go.khulnasoft.com/pkg/log"
	"go.khulnasoft.com/pkg/module"
	"go.khulnasoft.com/pkg/plugin"
	"go.khulnasoft.com/pkg/types"
	"go.khulnasoft.com/pkg/version"
	"go.khulnasoft.com/pkg/version/app"
	vexrepo "go.khulnasoft.com/pkg/vex/repo"
	xstrings "go.khulnasoft.com/pkg/x/strings"
)
...
func NewApp() *cobra.Command {
	...
	if cmd, _, _ := rootCmd.Find([]string{"aws"}); cmd == cmd.Root() { // "tunnel aws" not installed
		rootCmd.AddCommand(&cobra.Command{
			Hidden: true,
			Long:   "Tunnel AWS is now available as an optional plugin. See go.khulnasoft.com/tunnel-aws for details.",
			Use:    "aws",
		})
	}
...
func NewClientCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			log.Warn("'client' subcommand is deprecated now. See https://go.khulnasoft.com/tunnel/discussions/2119")
...
func NewPluginCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ tunnel plugin install go.khulnasoft.com/tunnel-plugin-referrer`,
			SilenceErrors:         true,
			SilenceUsage:          true,
